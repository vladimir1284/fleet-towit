...
async function main() {
	...
	if(!existingAdminTenant){
		...
		const admin_user_0 = await createTenantUser({email: 'gissell1184@gmail.com', userRole: Role.ADMIN, tenantId: admin_tenant.id})
		...
		const admin_user_1 = await createTenantUser({email: 'luis.ulloa75360@gmail.com', userRole: Role.ADMIN, tenantId: existingAdminTenant.id})
	}
}
main()
	.then(async () => {
