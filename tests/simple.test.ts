...
test('Create new tenant using bypass RLS prisma client', async () => {
	let promise = bypassPrisma.tenant.create({
		data: { name: 'BypassPrisma', email: 'example@company.com' }
	});
...
test('Testing Select to Tenant table without any context. Expected: 0-length result array', async () => {
...
test('Creates new user on Tenant', async () => {
	const newPrisma = tenantPrisma(tenant.id);
	let promise: any = prisma.user.create({
		data: {
			name: 'Test'
		}
	});
...
test("Delete tenant using tenant's ID as context", async () => {
	const newPrisma = tenantPrisma(tenant.id);
