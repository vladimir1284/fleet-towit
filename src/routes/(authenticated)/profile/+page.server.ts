...
export const load = (async({ locals }) => {
  ...
  if (session?.user?.id) {
    ...
    const data: editUserSchemaType = {id: user?.id, name: user?.name, email: user?.email||'', image: user?.image};
...
export const actions = {
    default: async ({ request }) => {
      ...
      await editUser({userId: form.data.id||'', email: form.data.email, name: form.data.name||undefined, image: form.data.image||undefined})
