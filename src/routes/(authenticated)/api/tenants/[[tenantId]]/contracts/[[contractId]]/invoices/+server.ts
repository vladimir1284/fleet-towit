...
export const GET: RequestHandler = async ({ locals, params }) => {
	...
	try {
		...
		const invoices = await reqAccountApi.getInvoicesForAccount({ accountId });
		// const invoices = await customReqInvoiceApi.getLatestInvoices({ limit: limit, audit: 'MINIMAL' });
