...
export const createTenantUser = async({email, tenantId, userRole=Role.STAFF}: createUserType) => {
    let user = await bypassPrisma.user.findUnique({where:{email: email}})
    if (!user) {
        user = await bypassPrisma.user.create({data: {email: email}})
    }
    const tenantUser = await bypassPrisma.tenantUser.create({data:{
        userId: user.id,
        tenantId: tenantId,
        role: userRole
    }})
...
export const updateTenant = async({tenantId, name, email}: editTenantType) => {
    const tenant = await bypassPrisma.tenant.update({where:{id: tenantId}, data:{name: name, email: email}})
    return tenant
}

export const updateTenantUser = async({tenantUserId, email, tenantId, userRole}: editUserType) => {
    let user = await bypassPrisma.user.findUnique({where:{email: email}})
    if (!user) {
        user = await bypassPrisma.user.create({data: {email: email}})
    } 
...
export const deleteUser = async({tenantUserId}: {tenantUserId: string}) => {
    ...
    const restTenantUsers = await bypassPrisma.tenantUser.findMany({where: {userId: tenantUser?.userId}})
