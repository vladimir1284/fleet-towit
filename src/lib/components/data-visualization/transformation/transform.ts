...
const wordifyCamel = (value: string) => {
  ...
  for (let i = 1; i < value.length - 1; i++) {
    if (isAlphabetic(value[i - 1]) && value[i - 1] !== value[i - 1].toUpperCase() && isAlphabetic(value[i]) && value[i] === value[i].toUpperCase() && isAlphabetic(value[i + 1]) && value[i + 1] !== value[i + 1].toUpperCase()) {
      words.push(" ");
    }
