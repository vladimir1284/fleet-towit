...
import { TieredBlockFromJSON, TieredBlockFromJSONTyped, TieredBlockToJSON } from './TieredBlock';
...
export function TierFromJSONTyped(json: any, ignoreDiscriminator: boolean): Tier {
	...
	return {
		...
		fixedPrice: !exists(json, 'fixedPrice')
			? undefined
			: (json['fixedPrice'] as Array<any>).map(PriceFromJSON),
		recurringPrice: !exists(json, 'recurringPrice')
			? undefined
			: (json['recurringPrice'] as Array<any>).map(PriceFromJSON),
		blocks: !exists(json, 'blocks')
			? undefined
			: (json['blocks'] as Array<any>).map(TieredBlockFromJSON)
	};
}
	...
	return {
		...
		fixedPrice:
			value.fixedPrice === undefined
				? undefined
				: (value.fixedPrice as Array<any>).map(PriceToJSON),
		recurringPrice:
			value.recurringPrice === undefined
				? undefined
				: (value.recurringPrice as Array<any>).map(PriceToJSON),
		blocks:
			value.blocks === undefined ? undefined : (value.blocks as Array<any>).map(TieredBlockToJSON)
	};
}
