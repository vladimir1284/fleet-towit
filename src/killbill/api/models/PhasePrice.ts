...
import { UsagePriceFromJSON, UsagePriceFromJSONTyped, UsagePriceToJSON } from './UsagePrice';
...
export function PhasePriceFromJSONTyped(json: any, ignoreDiscriminator: boolean): PhasePrice {
	...
	return {
		...
		usagePrices: !exists(json, 'usagePrices')
			? undefined
			: (json['usagePrices'] as Array<any>).map(UsagePriceFromJSON)
	};
}
...
export function PhasePriceToJSON(value?: PhasePrice | null): any {
	...
	return {
		...
		usagePrices:
			value.usagePrices === undefined
				? undefined
				: (value.usagePrices as Array<any>).map(UsagePriceToJSON)
	};
}
